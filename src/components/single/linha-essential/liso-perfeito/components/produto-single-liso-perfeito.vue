            ...
            <div class="row">
                <div class="col-md-7" id="sliderProductSingle" ref="sliderProductSingle">
                        ...
                        <div class="carousel-inner" id="carouselInnerSlider">
                            <div class="imagens-plataforma" v-if="sabores == 0">
                                <div v-if="kitPotes == 'KITCOMPLETO'">
                                     <!-- <img src="@/assets/img/linha-essential/linhas/liso-perfeito/produtos/kit-completo-liso-perfeito.jpg" class="img-fluid" alt=""> -->
                                    <div v-for="(imagem, index) in galeriaImagens" :key="index" :class="['carousel-item',{active: index==2}]" > 
                                        <img :src="imagem.arquivos.big" class="img-fluid" alt="">
                                    </div>
                                </div>
                    ...
                    <div class="desk">
                        ...
                        <BeneficiosEssentialSingle nomeLinha="SGLisoPerfeito" />
                    </div>
                </div>
                ...
                <div class="col-md-5" id="InfoProduto">
                    ...
                    <div class="descricao mb-4">
                        <div class="item-desc pb-1 d-flex align-items-center" v-for="(topico, index) in topicos" :key="index">
                            <img class="mr-1" src="@/assets/img/single/selecionado.png" alt="" />
                            <p>{{topico}}</p>
                        </div>
                    </div>
                    ...
                    <!-- Mascara 1 pote -->
                    <div class="prices" v-if="kitPotes == 'KITSPRAY'">
                        <p class="priceOld"> <s> {{ saboresKit.kitspray.precosGerais.precoDe.toLocaleString('pt-BR', { style: 'currency', currency: 'BRL' }) }} </s> </p>
                        <div class="d-flex align-items-center justify-content-between">
                            <p class="currentPrice"> <strong>{{ (saboresKit.kitspray.precosGerais.precoPor).toLocaleString('pt-BR', { style: 'currency', currency: 'BRL' }) }}</strong> </p>
                            <p class="discount"> ECONOMIZE <br> <strong> {{ (saboresKit.kitspray.precosGerais.precoDe - saboresKit.kitspray.precosGerais.precoPor).toLocaleString('pt-BR', { style: 'currency', currency: 'BRL' }) }} </strong></p>
                        </div>
                        <div class="d-flex align-items-center mt-3 mb-2">
                            <p class="priceNew">Ou 8x de <strong> {{ (saboresKit.kitspray.precosGerais.precoPor/8).toLocaleString('pt-BR', { style: 'currency', currency: 'BRL' }) }}</strong> </p>
                        </div>
                    </div>

                    <!-- Mascara 1 pote -->
                    <div class="prices" v-if="kitPotes == 'KITMASCARA'">
                        <p class="priceOld"> <s> {{ saboresKit.kitmascara.precosGerais.precoDe.toLocaleString('pt-BR', { style: 'currency', currency: 'BRL' }) }} </s> </p>
                        <div class="d-flex align-items-center justify-content-between">
                            <p class="currentPrice"> <strong>{{ (saboresKit.kitmascara.precosGerais.precoPor).toLocaleString('pt-BR', { style: 'currency', currency: 'BRL' }) }}</strong> </p>
                            <p class="discount"> ECONOMIZE <br> <strong> {{ (saboresKit.kitmascara.precosGerais.precoDe - saboresKit.kitmascara.precosGerais.precoPor).toLocaleString('pt-BR', { style: 'currency', currency: 'BRL' }) }} </strong></p>
                        </div>
                        <div class="d-flex align-items-center mt-3 mb-2">
                            <p class="priceNew">Ou 8x de <strong> {{ (saboresKit.kitmascara.precosGerais.precoPor/8).toLocaleString('pt-BR', { style: 'currency', currency: 'BRL' }) }}</strong> </p>
                        </div>
                    </div>
                    ...
                    <div class="group-kits my-3">
                        ...
                        <form>
                            <label class="check-label w-100 d-flex align-items-center my-2 position-relative" for="kit2Potes" @click="changeKit($event), idLista = 130">
                                <div class="kit-potes d-flex align-items-center">
                                    ...
                                    <div class="d-flex flex-column">
                                        <p class="name-kit"> Kit Completo Liso Perfeito</p>
                                        <p class="description-kit">Shampoo, condicionador, máscara <br class="mob"> e fluído termoativado</p>
                                    </div>
                                </div>
                            ...
                            <label class="check-label w-100 d-flex align-items-center my-2 position-relative" for="kit1" @click="changeKit($event), idLista = 131"> 
                                <div class="kit-potes d-flex align-items-center">
                                    <div class="circ-select">
                                        <input class="check-input mx-3" type="radio" id="kit1" name="kit" value="KITBASICO" v-model="kitPotes">
                                    </div>
                                    <div class="d-flex flex-column">
                                        <p class="name-kit"> Essential Liso Perfeito</p>
                            ...
                            <label class="check-label w-100 d-flex align-items-center my-2 position-relative" for="kitspray" @click="changeKit($event), idLista = 132">
                                <div class="kit-potes d-flex align-items-center">
                                    <div class="circ-select">
                                        <input class="check-input mx-3" type="radio" id="kitspray" name="kit" value="KITSPRAY" v-model="kitPotes">
                                    </div>
                                    <div class="d-flex flex-column">
                                        <p class="name-kit">Spray Liso Perfeito</p>
                                    </div>
                                </div>
                            </label>
                            <label class="check-label w-100 d-flex align-items-center my-2 position-relative" for="kitmascara" @click="changeKit($event), idLista = 135">
                                <div class="kit-potes d-flex align-items-center">
                                    <div class="circ-select">
                                        <input class="check-input mx-3" type="radio" id="kitmascara" name="kit" value="KITMASCARA" v-model="kitPotes">
                                    </div>
                                    <div class="d-flex flex-column">
                                        <p class="name-kit">Máscara Liso Perfeito</p>
                                    </div>
                                </div>
                            </label>
                            <label class="check-label w-100 d-flex align-items-center my-2 position-relative" for="kitshampoo" @click="changeKit($event), idLista = 133">
                                <div class="kit-potes d-flex align-items-center">
                                    <div class="circ-select">
                                        <input class="check-input mx-3" type="radio" id="kitshampoo" name="kit" value="KITSHAMPOO" v-model="kitPotes">
                                    </div>
                                    <div class="d-flex flex-column">
                                        <p class="name-kit">Shampoo Liso Perfeito</p>
                                    </div>
                                </div>
                            </label>
                            <label class="check-label w-100 d-flex align-items-center my-2 position-relative" for="kitcondicionador" @click="changeKit($event), idLista = 134">
                                <div class="kit-potes d-flex align-items-center">
                                    <div class="circ-select">
                                        <input class="check-input mx-3" type="radio" id="kitcondicionador" name="kit" value="KITCONDICIONADOR" v-model="kitPotes">
                                    </div>
                                    <div class="d-flex flex-column">
                                        <p class="name-kit">Condicionador Liso Perfeito</p>
                                    </div>
                                </div>
                            </label>
                        </form>
                    ...
                    <div class="mob">
                        <div class="d-flex align-items-center justify-content-between pt-4">
                            <img class="img-fluid" src="@/assets/img/single/SELOS.png" alt="Compra segura"/>
                        </div>
                    </div>
                </div>
            </div>
     ...
     props: {
        ...
        data() {
            return {
                ...
                nome: 'Home Care Essential - Liso Perfeito',
                breadcrumb: '',
                galeriaImagens: [],
                precoDe: '0',
                precoPor: 0,
                ...
                topicos: {
                    1: 'Oléo de argan, arginina e pantenol;',
                    2: 'Brilho e hidratação intensos;',
                    3: 'Liso prolongado;',
                    4: 'Frizz controlado.',
                },
                saboresKit: {
        ...
        methods: {
            changeKit(event) {
                ...
                this.changeSaborOption(this.kitPotes)
                this.scrollToDiv()
            },
...
<style scoped>
    ...
    .selecionado {
        ...
        border: 1px solid #FF4085 !important;
    }
