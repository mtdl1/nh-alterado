...
export default {
  ...
  computed: {
    ...
    ...mapState({
      ...
      dadosCarrinho: state => state.head.dadosCarrinho,
      dadosUsuario: state => state.head.dadosUsuario
    })
  },
  watch: {
    ...
    dadosPageAtual(val) {
      if (val != null) this.getData(val);
    },
    dadosCarrinho(val) {

      if (val != null) this.setCarrinho(val);
    },
    dadosUsuario(val) {
      if (val != null) this.setUsuario(val);
    },
    config(newVal) {
      if (newVal != null) {
  ...
  methods: {
    ...mapMutations(['changeVariaveisPublicas']),
    setAtributos(attr) {
      //ATRIBUTOS
      var valores = [];
      ...
      if(attr.simples){
        ...
        attr.simples.valores.forEach(value => {
          valores.push({
            valor: value.label
          });
        });
        ...
        this.wapstore.produto.atributos.simples.atributo = attr.simples.nome;
      ...
      if(attr.unico){
        //OBTÉM TODOS OS VALORES
        valores = [];
        attr.unico.valores.forEach(value => {
          valores.push({
            valor: value.label
          });
        });
        //ATRIBUI O NOME E VALORES
        this.wapstore.produto.atributos.unico.atributo = attr.unico.nome;
    ...
    print(){
      ...
      if(typeof window !== 'undefined'){
        ...
        this.changeVariaveisPublicas(this.responsePagina);
      }
    },
    setUsuario(responseUsuario){
      ...
      //CONTROLE
      this.usuario = true;
      this.success = this.usuario && this.pagina;
    },
    ...
    setPagina(responsePagina){
      ...
      switch (responsePagina.estrutura.nivel) {
        ...
        case 'page':
          return this.setPaginaInstitucional();
      }
    },
    setPaginaHome(){
      //DADOS DA HOME
    ...
    setPaginaDetalheProduto(responseDetalhe){
      ...
      //ATRIBUTOS
      this.setAtributos(responseDetalhe.conteudo.atributos);
    ...
    setPaginaInstitucional(){
      //DADOS DA HOME
      this.wapstore.pagina.nome      = "Outro";
      this.wapstore.pagina.secao     = "outro";
      this.wapstore.pagina.subsecao  = "outro";
      this.wapstore.pagina.total     = 1;
      this.wapstore.pagina.atual     = 1;
  ...
  created(){
    ...
    //DADOS DO CARRINHO
    if (this.dadosCarrinho != null) this.setCarrinho(this.dadosCarrinho);

    //DADOS DA PÁGINA
    if (this.dadosPageAtual != null) this.getData(this.dadosPageAtual);
