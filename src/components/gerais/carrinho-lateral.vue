    ...
    <div class="carrinhoLateral-mainContainer">
      ...
      <transition name="fadeNoDelay">
        <div class="carrinhoLateral-preloader" v-if="loader">
          <img src="@/assets/img/geral/preloader.svg" v-if="!masculino" alt="">
          <img src="@/assets/img/geral/preoloader-masculino.svg" v-if="masculino" alt="">
        </div>
      </transition>

      <!-- TOPO CARRINHO-->
      <div class="carrinhoLateral-topo">
        <span class="carrinhoLateral-t1" style="color: #00b5dc;" v-if="masculino">Carrinho ({{ qtdItens }})</span>
        <span class="carrinhoLateral-t1" style="color: #ff4289;"  v-if="!masculino">Carrinho ({{ qtdItens }})</span>
        <button :class="['carrinhoLateral-fechar', {bgMasc:masculino, bgFem:!masculino} ] " @click="closeCarrinho()">
      ...
      <div class="carrinhoLateral-prods">
        ...
        <div v-if="produtos && qtdItens > 0">
          <div class="carrinhoLateral-prods-item" v-for="(produto, i) in produtos" :key="i">
            ...
            <div class="carrinhoLateral-prods-remove"><img src="@/assets/img/delete.png" @click="rmvItem(i)" style="height:auto; cursor:pointer;" /></div>
      ...
      <template>
        <div class="carrinhoLateral-rodape">
          ...
          <div class="carrinhoLateral-rodape-bts">
            <div :class="['w-100',{disabled: subtotal.valor > 1000}]">
              <a class="carrinhoLateral-rodape-comprar w-100" :href="`${$siteUrl}checkout/carrinho/`">Finalizar compra</a>
              <!-- <a class="carrinhoLateral-rodape-comprar w-100"      v-if="telefoneEnviado"     :href="`${$siteUrl}checkout/carrinho/`">Finalizar compra</a>
              <button class="carrinhoLateral-rodape-comprar w-100" v-else @click="openModalForm()">Finalizar compra</button>      -->
            </div>
            <!-- <router-link to="/produtos" class="carrinhoLateral-rodape-fechar w-100">
              <span @click="closeCarrinho()"> Continue Comprando </span>
            </router-link> -->
          </div>

          <p class="alertMax" v-if="subtotal.valor > 1000"> Valor máximo do pedido é de R$1000</p>
        </div>
      </template>
      <!-- <transition name="slideLeft">
        <ModalEmailCarrinho v-show="showModalForm" @close-Modal="fecharModal" :dadosCarrinho="produtos"/>   
      </transition> -->
    </div>
  </section>
</template>
...
<script>
  ...
  // import ModalEmailCarrinho from '@/components/gerais/modais/modalEmailCarrinho.vue'  
  ...
  export default {
    ...
    created() {
      ...
      this.dadosCarrinhoLateral = this.produtos;
      ...
      localStorage.getItem('TelefoneEnviado') ? this.telefoneEnviado = true : this.telefoneEnviado = false; 
    ...
    mounted() {
      ...
      // Identifica o diretorio do site após o dominio
			this.url = window.location.href;
			this.url = this.url.split("/");
			this.url = this.url[3]
			if(this.url == "newhair-masculino" || this.url == "produtos-masculino" || this.url == "new-hair-gummy-uva-verde-vitamina-em-goma-para-cabelos.html" || this.url.includes("new-hair-caps-masculino") ){
				this.masculino = true;
			}
...
<style scoped>
  ...
  .carrinhoLateral-fechar:hover path{
    fill: #ff4289 !important;
  }

  .carrinhoLateral-fechar:hover svg{
    fill: #818181;
  }
  .carrinhoLateral-fechar.bgMasc:hover{
    border: 1px solid #00b5dc;
  }

  .carrinhoLateral-fechar.bgMasc:hover path{
    fill: #00b5dc !important;
  }
